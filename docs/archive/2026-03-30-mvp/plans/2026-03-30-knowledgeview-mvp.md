    ...
    if (selectedEdgeId) {
        ...
        return (
            <div className="flex h-full w-[260px] flex-col border-l p-4">
                <span className="text-muted-foreground mb-3 text-xs font-medium uppercase">
                    선택된 트리플
                </span>
        ...
        <div className="flex h-full w-[260px] flex-col border-l">
            <ScrollArea className="flex-1 p-4">
                ...
                <div className="mt-2 mb-3">
                    ...
                    <div className="bg-muted/30 rounded-md border px-3 py-1.5 text-sm">
                        {node.label}
                    </div>
                </div>
                ...
                <span className="text-muted-foreground mb-2 text-xs font-medium uppercase">
                ...
                {connectedTriples.map((t) => {
                    ...
                    return (
                        ...
                        >
                            ...
                            <span
                                className={
                                    isViolated ? "text-red-400" : "text-primary"
                                }
                            >
                                {t.predicate}
                            </span>
...
export function RuleForm({ open, onClose, onSubmit, graph }: RuleFormProps) {
    ...
    const [ruleType, setRuleType] = useState<
        "constraint" | "inference" | "validation"
    >("constraint");
    const [nodeType, setNodeType] = useState("");
    const [predicate, setPredicate] = useState("");
    const [operator, setOperator] =
        useState<RuleCondition["operator"]>("must_have");
                ...
                <div className="space-y-4 py-4">
                    ...
                    <div>
                        ...
                        <ToggleGroup
                            ...
                            value={ruleType}
                            onValueChange={(v) =>
                                v && setRuleType(v as typeof ruleType)
                            }
                        >
                            {RULE_TYPES.map((t) => (
                                <ToggleGroupItem
                    ...
                    <div className="rounded-lg border p-4">
                        ...
                        <div className="mt-2 flex flex-wrap items-center gap-2 text-sm">
                            <Select
                                value={operator}
                                onValueChange={(v) =>
                                    setOperator(v as RuleCondition["operator"])
                                }
                            >
                        ...
                        {operator === "conflicts_with" && (
                            <div className="mt-2 flex flex-wrap items-center gap-2 text-sm">
                                ...
                                <Select
                                    value={conflictPredicate}
                                    onValueChange={setConflictPredicate}
                                >
                                    <SelectTrigger className="w-[150px]">
                                        <SelectValue placeholder="관계" />
                                    </SelectTrigger>
            ...
            <div className="relative flex-1">
                ...
                <div className="absolute top-3 right-3 z-10 flex gap-2">
                    ...
                    <Button
                        size="sm"
                        variant="outline"
                        onClick={() => setTripleFormOpen(true)}
                    >
                        + 트리플
                    </Button>
                    {isDirty && (
